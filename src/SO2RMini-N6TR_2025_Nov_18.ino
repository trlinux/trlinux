...
// under the maximum.  
//
// 20-Nov-2025: Some changes to 2BSIQ footswitch mode.  Made the length of the pulse sent to
// stop a tranmission programmable (was fixed at 25 ms - which isn't always long enough for a
// K4).  The length is now set by the top six bits of the Footswitch mode command byte.  This
// gives values up to 253 ms - with 4 ms resolution.  Also - bumped up the version # to assure
// that the TR program and the arduino are in sync.  
...
int FootswitchMode = 1;                 // 0-do nothing 1=control PTT  2=2BSIQ mode
int StopTransmissionPulseLength = 0;    // 0 to 252 ms in 4 ms steps
...
// New in November 2025, we use the bottom 4 bits to identify the FootswitchMode
// and the other 4 bits are used to identify the length of the PTT pulse used
// to stop a memory transmission when using the special 2BSIQ mode.  The 6 bit
// value is multiplied by 4 milliseconds.  A value of zero will disable the pulse

        case 0x19:
          {
            FootswitchMode = hostchar & 0x03;  // 01 = footswitch control PTT  02 is special 2BSIQ SSB FS mode  03 is CW lockout mode
            StopTransmissionPulseLength = hostchar & 0xFC;
            break;
          }
...

{
  ...
  // assert the PTT for the corresponding radio when pressed.  It will also generate a 25 millisecond
  // pulse on the PTT for the other radio before starting the PTT to abort any message that might be
  // playing on that radio.  November 2025 - increased the pulse to 40 ms as sometimes it was
  // not being seen.  
  ...
  if (FootswitchMode == FS_Dual)
    ...
    if (digitalRead (ditInput) == LOW)          // Radio One Footswitch pressed
      ...
      if (DebouncedFootswitch1Pressed == false)  // Stop any message playing on Radio two
        {
        RadioTwoStopTransmitPTT = StopTransmissionPulseLength;  // used to be 25 or 40 

        // Only set the PTT if we have a value > 0
    ...
    if (digitalRead (dahInput) == LOW)    // Radio Two Footswitch pressed
      ...
      if (DebouncedFootswitch2Pressed == false)  // Stop any message playing on Radio One
        {
        RadioOneStopTransmitPTT = StopTransmissionPulseLength;
