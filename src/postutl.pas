...
VAR CabrilloFrequencyString: STRING;
    FirstEntry, SecondEntry, CabrilloDate, CabrilloTime, CabrilloCall: Str20;
    CabrilloFrequency, FrequencyValue, Address: INTEGER;

    BEGIN
    IF NumberLongLogFileEntries = 0 THEN Exit;

    { We need to get the Cabrillo frequency - N1MM put more spaces between the QSO:
      entry and the frequency than TR does.  So - have to accomodate for that.  We
      reconstruct the Cabrillo entry to not have these extra spaces }

    FirstEntry := RemoveFirstString (CabrilloString);  { Likely QSO: }
    SecondEntry := RemoveFirstString (CabrilloString); { Likely Frequency }

    { Make 160, 80 and 40meter freq five characters }

    IF Length (SecondEntry) = 4 THEN SecondEntry := ' ' + SecondEntry;

    CabrilloString := FirstEntry + ' ' + SecondEntry + ' ' + CabrilloString;

    CabrilloFrequencyString := CabrilloString;
    RemoveFirstString (CabrilloFrequencyString);  { Remove QSO: }
    CabrilloFrequencyString := RemoveFirstString (CabrilloFrequencyString);  { Frequency }
...
VAR LongLogFileName, OutputFileName, CabrilloFileName, ADIFFileName: Str80;
    ...
    WHILE NOT Eof (ADIFFileRead) DO
        ...
        WriteLn (Copy (ADIFFileString, 1, 70));
...
FUNCTION GetIntegerMinutes (TimeString: STRING): INTEGER;

VAR HourString, MinuteString: STRING;
    Hour, Minute: INTEGER;
    ...
    GetIntegerMinutes := (Hour * 60) + Minute;
    END;



FUNCTION ElaspedMinutes (TXTime, TimeString: STRING): INTEGER;

{ Returns TimeString - TXTime accounting for going over midnight }

VAR TXTimeInteger, TimeInteger: INTEGER;

    BEGIN
    TXTimeInteger := GetIntegerMinutes (TXTime);
    TimeInteger := GetIntegerMinutes (TimeString);

    IF TXTimeInteger > TimeInteger THEN  { Went over a date change }
        TimeInteger := TimeInteger + (24 * 60);

    ElaspedMinutes := TimeInteger - TXTimeInteger;
    END;



PROCEDURE CheckCQWWMSTXIDs;

VAR OutputFileName, InputFileName: Str80;
    OriginalCabrilloString, FileString: STRING;
    OutputFile, FileRead: TEXT;
    NumberViolations: INTEGER;
    TXIDString, TimeString: Str80;
    TX0Time, TX1Time: STRING;
    Frequency: LONGINT;
    TX0Band, TX1Band, Band: BandType;

    BEGIN
    ClearScreenAndTitle ('VERIFY CQWW MultiSingle TXIDs');
    WriteLn ('This procedure will look at your transmitter IDs in your Cabrillo');
    WriteLn ('file and inform you if there are any band change violations.  This');
    WritELn ('is intended to be used for the multisingle category in the CQ WW.');
    WriteLn;
    WriteLn ('Note - this does not confirm that QSOs on the mult radio are all mults.');
    WriteLn;
    ...
    OutputFileName := GetResponse ('Enter filename to save output to (none to abort) : ');
    ...
    WHILE NOT Eof (FileRead) DO
        ...
        IF RemoveFirstString (FileString) = 'QSO:' THEN
            BEGIN
            Frequency := RemoveFirstLongInteger (FileString);
            RemoveFirstString (FileString);  { get rid of mode }
            Band := GetBandFromCabrilloFrequency (Frequency);
            ...
            IF TXIDString = '0' THEN
                IF TX0Band <> Band THEN
                    IF TX0Band = NoBand THEN   { First QSO with this TX }
                        BEGIN
                        TX0Band := Band;
                        TX0Time := TimeString;
                        END
                    ELSE
                        IF ElaspedMinutes (TX0Time, TimeString) >= 10 THEN  { Legal band change }
                            BEGIN
                            TX0Band := Band;
                            TX0Time := TimeString;
                            END
                        ELSE
                            ...
                            { Change the TX ID of the Cabrillo File to ? }

                            OriginalCabrilloString [Length (OriginalCabrilloString)] := '?';
                            END;

            IF TXIDString = '1' THEN
                IF TX1Band <> Band THEN
                    IF TX1Band = NoBand THEN   { First QSO with this TX }
                        BEGIN
                        TX1Band := Band;
                        TX1Time := TimeString;
                        END
                    ...
                    ELSE
                        IF ElaspedMinutes (TX1Time, TimeString) >= 10 THEN  { Legal band change }
                            BEGIN
                            TX1Band := Band;
                            TX1Time := TimeString;
                            END
        ...
        WriteLn (OutputFile, OriginalCabrilloString);
        END;

    WriteLn ('There were ', NumberViolations, ' band changes violations found.');
    WriteLn ('QSOs that are violations are marked with TXID of ? in the new file.');
...
VAR RunFileRead, CabrilloFileRead, CabrilloFileWrite: TEXT;
    ...
    CabrilloBand, RunBand, NextRunBand: BandType;

    BEGIN
    ClearScreenAndTitle ('ASSIGN TX ID');
    WriteLn ('This procedure will assign TX ID zero for the run transmitter and ID one for the');
    WritELn ('mult radio based upon the data in the file run.txt.  This file identifies the');
    WriteLn ('start date and time for what band the run TX is on.  The format is : ');
    WriteLn;
    WriteLn ('2024-11-23 0000 28000');
    WriteLn;

    IF NOT OpenFileForRead (RunFileRead, 'run.txt') THEN
        BEGIN
        WriteLn ('No file run.txt found.');
        WaitForKeyPressed;
        Exit;
        END;

    CabrilloInputFileName := GetResponse ('Enter filename of Cabrillo log to modify (none to abort) : ');

    IF CabrilloInputFileName = '' THEN
        BEGIN
        Close (RunFileRead);
        Exit;
        END;

    IF NOT OpenFileForRead (CabrilloFileRead, CabrilloInputFileName) THEN
        BEGIN
        Close (RunFileRead);
        WriteLn ('Unable to open ', CabrilloInputFileName);
        WaitForKeyPressed;
        Exit;
        END;

    CabrilloOutputFileName := GetResponse ('Enter filename of Cabrillo log output (none to abort) : ');

    IF CabrilloOutputFileName = '' THEN
        BEGIN
        Close (CabrilloFileRead);
        Close (RunFileRead);
        Exit;
    ...
    OpenFileForWrite (CabrilloFileWrite, CabrilloOutputFileName);

    { Files all open }

    ReadLn (RunFileRead, RunFileData);
    RunFileDataDate := RemoveFirstString (RunFileData);
    RunFileDataTime := RemoveFirstString (RunFileData);
    FrequencyString := RemoveFirstString (RunFileData);

    IF FrequencyString = '1800' THEN RunBand := Band160;
    IF FrequencyString = '3500' THEN RunBand := Band80;
    IF FrequencyString = '7000' THEN RunBand := Band40;
    IF FrequencyString = '14000' THEN RunBand := Band20;
    IF FrequencyString = '21000' THEN RunBand := Band15;
    IF FrequencyString = '28000' THEN RunBand := Band10;

    { Run band is set for the start of the contest.  Now - read the next entry
      so we know when to change it. }

    IF NOT Eof (RunFileRead) THEN
        BEGIN
        ReadLn (RunFileRead, RunFileData);
        RunFileDataDate := RemoveFirstString (RunFileData);
        RunFileDataTime := RemoveFirstString (RunFileData);
        ...
        IF FrequencyString = '1800' THEN NextRunBand := Band160;
        IF FrequencyString = '3500' THEN NextRunBand := Band80;
        IF FrequencyString = '7000' THEN NextRunBand := Band40;
        IF FrequencyString = '14000' THEN NextRunBand := Band20;
        IF FrequencyString = '21000' THEN NextRunBand := Band15;
        IF FrequencyString = '28000' THEN NextRunBand := Band10;

        WriteLn ('Next run on ', RunFileDataDate, ' at ', RunFileDataTime);
        END
    ELSE
        RunFileDataDate := 'XXXXX';  { Will never match a QSO }

    { Have RunFileDataDate and RunFileDataTime set to next transition
      which will be to NextRunBand }

    WHILE NOT Eof (CabrilloFileRead) DO
        BEGIN
        ReadLn (CabrilloFileRead, CabrilloString);

        OriginalCabrilloString := CabrilloString;

        IF RemoveFirstString (CabrilloString) = 'QSO:'  THEN
            BEGIN
            FrequencyString := RemoveFirstString (CabrilloString);
            RemoveFirstString (CabrilloString);  { get rid of mode }
            CabrilloDateString := RemoveFirstString (CabrilloString);
            CabrilloTimeString := RemoveFirstString (CabrilloString);

            Val (FrequencyString, CabrilloFrequency);

            IF CabrilloFrequency < 3000 THEN CabrilloBand := Band160 ELSE
            IF CabrilloFrequency < 6000 THEN CabrilloBand := Band80 ELSE
            IF CabrilloFrequency < 10000 THEN CabrilloBand := Band40 ELSE
            IF CabrilloFrequency < 15000 THEN CabrilloBand := Band20 ELSE
            IF CabrilloFrequency < 22000 THEN CabrilloBand := Band15 ELSE
                CabrilloBand := Band10;

            { First check for new run band }

            IF (CabrilloDateString = RunFileDataDate) AND
               (CabrilloTimeString = RunFileDataTime) AND
               (CabrilloBand = NextRunBand) THEN
                   BEGIN
                   WriteLn ('Found new run band');
            ...
            { Now if the CabrilloBand = RunBand, set TX ID to zero.  Otherwise
              set to one. }

            IF CabrilloBand = RunBand THEN
                OriginalCabrilloString [Length (OriginalCabrilloString)] := '0'
            ELSE
                OriginalCabrilloString [Length (OriginalCabrilloString)] := '1'
            END;

        { Write new string to output file }

        WriteLn (CabrilloFileWrite, OriginalCabrilloString);
        END;
...
VAR Key: CHAR;
    ...
    WriteLn ('  Z - Check CQ WW MultiSingle TX IDs.');
    WriteLn;
    TextColor (Cyan);
    Write   ('  Enter command : ');
    ...
    REPEAT
        ...
        CASE Key OF
            ...
            'Z': BEGIN CheckCQWWMSTXIDs;     Exit; END;
